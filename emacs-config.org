...
****** CANCELLED eldoc-box                                        :CANCELLED:
       CLOSED: [2021-04-20 Di 07:27]
       :LOGBOOK:
       - State "CANCELLED"  from              [2021-04-20 Di 07:27] \\
         doesn't work at all
       :END:
       #+begin_src emacs-lisp
         (use-package eldoc-box)
       #+end_src
****** CANCELLED eldoc overlay                                    :CANCELLED:
       CLOSED: [2021-04-17 Sa 00:40]
       :PROPERTIES:
       :header-args:emacs-lisp: :tangle no
       :END:
       :LOGBOOK:
       - State "CANCELLED"  from              [2021-04-17 Sa 00:40] \\
         interferes with lsp
       - State "WAITING"    from              [2021-04-15 Do 00:10]
       :END:
       #+begin_src emacs-lisp
       ...
       #+end_src
****** CANCELLED tooltip-help                                     :CANCELLED:
       CLOSED: [2021-04-20 Di 08:09]
       :PROPERTIES:
       :header-args:emacs-lisp: :tangle no
       :END:
       :LOGBOOK:
       - State "CANCELLED"  from "STARTED"    [2021-04-20 Di 08:09] \\
         use in rgr/context minor mode stuff
       - State "STARTED"    from "CANCELLED"  [2021-04-20 Di 07:29]
       - State "CANCELLED"  from              [2021-04-16 Fr 23:39] \\
         unsupported functions
       - State "WAITING"    from              [2021-04-14 Mi 23:40] \\
         wait on clippy
       :END:
       #+begin_src emacs-lisp
         (use-package tooltip-help
           ...
           (defcustom rgr/elisp-popup-help-enabled nil "Non-nil means popup " :type 'boolean)
       ...
       #+end_src
****** CANCELLED clippy                                           :CANCELLED:
       CLOSED: [2021-04-16 Fr 23:39]
       :PROPERTIES:
       :header-args:emacs-lisp: :tangle no
       :END:
       :LOGBOOK:
       - State "CANCELLED"  from "WAITING"    [2021-04-16 Fr 23:39] \\
         too ugly
       - State "WAITING"    from              [2021-04-15 Do 00:10]
       :END:
       #+begin_src emacs-lisp
         (use-package clippy
           ...
           (defvar rgr/elisp-popup-timer nil "Timer for popping up elisp help when idle")
           (defvar rgr/elisp-popup-last nil "last symbol poppedup")
           :config
           ...
           (defun rgr/clippy()
             (interactive)
             (let ((tap (symbol-at-point)))
               (if (not (eq tap rgr/elisp-popup-last))
                   (if (fboundp tap)
                       (clippy-describe-function tap)
                     (clippy-describe-variable tap)))
               (setq-local rgr/elisp-popup-last tap)))
