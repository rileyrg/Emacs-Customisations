...
**** project-launch-external-script
...
#+begin_src bash :tangle "~/.emacs.d/bin/emacs-project-tmux"
  #!/usr/bin/env bash
  # Maintained in emacs-config.org
...
#+CAPTION: two tmux instances open for two different projects
#+NAME:   project-external-terminal-script
[[./images/project-launch-external-script.png]]

#+begin_src emacs-lisp :tangle (rgr/user-elisp-file "project-external-script.el")
...
:PROPERTIES:
:header-args:emacs-lisp: :tangle (rgr/user-elisp-file "init-info.el")
:END:
This tangles to its own init file [[file:etc/elisp/init-info.el][init-info.el]].
...
*** build
#+begin_src bash :tangle "~/.emacs.d/bin/emacs-build"
  #!/usr/bin/env bash
  # Maintained in emacs-config.org
  cd ~/development/projects/emacs/
  make -j$(nproc)
